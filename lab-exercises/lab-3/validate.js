...
async function routeExists() {
  ...
  assert.equal(res.statusCode, 200, `${URL} must respond with 200 response`);
...
async function layoutCheck() {
  ...
  const markers = RegExp(
    "<style>\\s+body\\s+{\\s+background:\\s+#333;\\s+margin:\\s+1\\.25rem\\s+}" +
  ...
  assert.match(html, markers, `${URL} must use the layout.hbs view to render`);
